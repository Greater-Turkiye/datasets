...
ORG_SETTING = ("Organisation settings → Actions → General → Workflow permissions → "
...
def compare_url(repo: str, branch: str) -> str:
    """The one-click "open a pull request from this branch" link."""
    return f"https://github.com/{repo}/compare/main...{branch}?expand=1"


def fallback_comment(issue: dict, repo: str, branch: str, body: str) -> str:
    """Posted on the issue when `gh pr create` fails — most often because the organisation does not
    allow Actions to open pull requests. The branch is already pushed, so one click finishes the job.

    The pull request body is offered in a `~~~` fence: it contains ``` fences of its own (the issue
    form renders URL fields as code blocks), so a backtick fence would not survive the paste.
    """
    return "\n".join([
        "### Taslak dal hazır, PR'ı bir insan açmalı / The draft branch is ready; a human has to open the PR",
        "",
        "Kayıt üretildi, `fmt` ve `validate` geçti ve dal itildi — ancak bu kuruluş GitHub Actions'ın pull "
        "request açmasına izin vermiyor. / The record was generated, passed `fmt` and `validate` and the branch "
        "was pushed, but this organisation does not allow GitHub Actions to create pull requests.",
        "",
        f"**Tek tıkla açın / Open it with one click:** {compare_url(repo, branch)}",
        "",
        f"Dal / Branch: `{branch}`",
        "",
        "### Bir inceleyenin doğrulaması gerekenler / What a reviewer must confirm",
        "",
        *[f"- [ ] {c}" for c in REVIEWER_CHECKLIST],
        "",
        "<details><summary>PR açıklaması için hazır gövde / Ready-made pull request body</summary>",
        "",
        "~~~markdown",
        body.replace("~~~", "~ ~ ~"),
        "~~~",
        ...
        "</details>",
        "",
        f"Bu geçici çözümü kaldırmak için / To remove this fallback, an organisation owner enables: {ORG_SETTING}. "
        "Bir `GT_BOT_TOKEN` sırrı eklemek de aynı işi görür. / Adding a `GT_BOT_TOKEN` secret does the same.",
    ])
...
def process(event: dict, out_dir: Path) -> dict[str, str]:
    ...
    body = pr_body(issue, rel, record, notes, repo)
    (out_dir / "pr_body.md").write_text(body, encoding="utf-8", newline="\n")
    (out_dir / "commit_msg.txt").write_text(commit_message(issue, record), encoding="utf-8", newline="\n")
    # Written up front so the workflow never has to build Markdown out of untrusted text in a shell.
    # It is only posted if `gh pr create` fails (e.g. the organisation blocks Actions from opening PRs).
    (out_dir / "fallback_comment.md").write_text(fallback_comment(issue, repo, branch, body),
                                                 encoding="utf-8", newline="\n")
    return {"status": "ok", "branch": branch, "record_path": rel.as_posix(), "record_id": record["id"],
            "region": record["regions"][0], "compare_url": compare_url(repo, branch),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--out-dir", default=".",
                        help="where to write pr_body.md / commit_msg.txt / fallback_comment.md / rejection.md")
